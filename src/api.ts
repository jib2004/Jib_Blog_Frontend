...
export const api = {
  ...
  login: async (email: string, password: string) => request<{ token?: string; access_token?: string; user_token?: string; userInfo?: ApiUser }>('/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) }),
  ...
  createBlog: async (post: Pick<ApiPost, 'title' | 'content'>, userId: string, token: string) => unwrap(await request<ApiPost>('/blog/', { method: 'POST', token, body: JSON.stringify({ ...post, user_id: userId }) })),
  updateBlog: async (id: string, post: Pick<ApiPost, 'title' | 'content'>, userId: string, token: string) => unwrap(await request<ApiPost>(`/blog/${id}`, { method: 'PUT', token, body: JSON.stringify({ ...post, user_id: userId }) })),
